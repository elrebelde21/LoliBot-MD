...
// CARPETA DONDE GUARDAR LOS ARCHIVOS DE NEDB
...
// NO USAMOS setAutocompactionInterval en cada colección, para evitar overhead
// Si quieres activarlo cada minuto, puedes descomentarlo:
...
async function migrate() {
  ...
  // AL FINALIZAR, COMPACTAMOS LOS ARCHIVOS .db PARA OPTIMIZAR TAMAÑO
  for (const category of categories) {
    if (collections[category]) {
      await new Promise((resolve, reject) => {
        collections[category].persistence.compactDatafile();
        // compactDatafile es sin callback, así que esperamos un poco
