...
export default async function (m, conn = { user: {} }) {
  // Obtener el nombre del remitente y del chat
  let _name = await conn.getName(m.sender);
  let sender = PhoneNumber('+' + m.sender.replace('@s.whatsapp.net', '')).getNumber('international') + (_name ? ' ~' + _name : '');
  let chat = await conn.getName(m.chat);
  let img;

  // Manejo de imágenes (si está habilitado)
  try {
    if (global.opts['img']) {
  ...
  let me = conn.user?.jid
    ...
    : 'Desconocido';
  const userName = conn.user?.name || 'Desconocido';

  // Determinar si es el bot principal de forma segura
  let isMainBot = false;
  try {
    isMainBot = conn.user?.jid && global.conn?.user?.jid ? conn.user.jid === global.conn.user.jid : false;
  } catch (e) {
    console.warn('No se pudo determinar si es el bot principal. Asumiendo que es un sub-bot:', e.message);
    isMainBot = false; // Si hay algún error, asumimos que es un sub-bot
  }
