...
console.log({
  ...
  '¿Hay conn?': !!conn,
  '¿isCode?': isCode,
  '¿Flag?': commandFlags && senderId ? commandFlags[senderId] : undefined,
  senderId,
  'Chat': m?.chat
})
...
if ([DisconnectReason.loggedOut,  DisconnectReason.badSession, DisconnectReason.restartRequired].includes(reason)) {
if (intentos < 5) {
console.log(chalk.yellow(`[🛑 SUB-BOT ${botId}] Conexion cerrada... Reintentando (${intentos}/5)`));
setTimeout(() => {
startSubBot(m, conn, caption, isCode, phone, chatId, {});
}, 3000);
} else {
console.log(chalk.red(`[❌ SUB-BOT ${botId}] Sesión inválida tras múltiples intentos. Eliminando sesión.`));
try {
fs.rmSync(sessionFolder, { recursive: true, force: true });
} catch (e) {
console.error(`[❌] No se pudo eliminar la sesión de ${botId}:`, e);
}
