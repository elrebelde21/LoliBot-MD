...
async function getDiskUsage() {
    try {
        ...
        const totalSize = await getFolderSize(path);
        const usedSize = await getUsedSize(path); 
        const freeSize = await getFreeSize(path); 
     
     return { total: totalSize, usado: usedSize, libre: freeSize };
    } catch (err) {
