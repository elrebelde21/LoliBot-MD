...
`.trim();
      ...
      // Verificar sub-bots apagados cada 5 minutos dentro de connectionUpdate
      setInterval(() => {
        ...
        for (const subBotId of subBotDirs) {
          ...
          if (subBotIndex === -1 || !global.conns[subBotIndex]?.user || global.conns[subBotIndex]?.ws?.readyState !== ws.OPEN) {
            ...
            try {
              const { state, saveCreds } = useMultiFileAuthState.sync(pathGataJadiBot); // Usar .sync para evitar await fuera de async
              ...
              creloadHandler(true).catch(console.error); // Usar función directamente sin await
