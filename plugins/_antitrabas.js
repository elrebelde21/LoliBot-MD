...
await conn.fakeReply(m.chat, `*[ ! ] Se detecto un mensaje que contiene muchos caracteres [ ! ]*`, '0@s.whatsapp.net', `${isBotAdmin ? 'Todavia existe la lergionVirgo? 🤣' : 'No soy administrador, no puedo hacer nada :/'}`, 'status@broadcast', null, fake)
...
setTimeout(() => { 
conn.fakeReply(m.chat, `Marcar el chat como leido ✓\n${"\n".repeat(400)}\n• El número : wa.me/${m.sender.split("@")[0]}\n• Alias : ${name}\n‼️Acaba de enviar un texto que contiene muchos caracteres que puede ocasionar fallos en los dispositivos`, '0@s.whatsapp.net', `Todavia existe la lergionVirgo? 🤣, sera expulsando aca no queremos rata 🐁`, 'status@broadcast', null, fake)
