...
let txt = '';
try {
const res = await db.query(`SELECT id FROM chats WHERE is_group = true AND joined = true AND bot_id = $1`, [botId]);
...
const bot = metadata.participants.find((u) => u.id?.includes(botNumber) || u.phoneNumber?.includes(botNumber)) || {};
